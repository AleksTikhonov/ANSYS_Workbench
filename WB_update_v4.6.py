...
projectfiledirpath=wdpath + '\\' + projectname + '_files' 
projecinsidepath=wdpath + '\\' + projectname + '_files\\dp0'
projectfilepath=wdpath + '\\' + projectname + '.wbpj'
projectarchivepath=wdpath + '\\' + projectname + '.wbpz'
csvpath=wdpath + '\\' + projectname + '.csv'
pattern = r"(P\d{1,3})\s-\s([a-zA-Z _.@0-9]+)\s?(\[.+?\])*"
pattern_2 = r"DP\s.*"
output_file_name = wdpath + '\\values\\values.csv'
res = []
val = {}
    ...
    for root, dirs, files in os.walk(projectfiledirpath):
        for file in files:
            if file.endswith('.res'):
                shutil.copy(os.path.join(root, file), os.path.join(wdpath + '\\main_results\\cfd', file))
                export_cfd_report()
            if file.endswith('.gtm') or file.endswith('.mshdb'):
                shutil.copy(os.path.join(root, file), os.path.join(wdpath + '\\main_results\\cfd\\mesh', file))                

def export_report():
    ExportReport(FilePath=wdpath + '\\main_results\\report.html')
    for root, dirs, files in os.walk(projecinsidepath):
        for file in files:
            if file.startswith('Post') and file.endswith('.png'):
                shutil.copy(os.path.join(root, file), os.path.join(wdpath + '\\pictures', file))
    try:
        shutil.copy(wdpath + '\\main_results\\report_images\\ProjectSchematic.png', wdpath + '\\pictures\\model_overview.png')
    except:
        pass
...
def make_dirs():
    try:
        os.mkdir(wdpath + '\\values')
    except OSError:
        pass
    try:
        os.mkdir(wdpath + '\\main_results')
    except OSError:
        pass
    try:
        os.mkdir(wdpath + '\\pictures')
    except OSError:
        pass
    try:
        os.mkdir(wdpath + '\\main_results\\cfd')
    except OSError:
        pass
    try:
        os.mkdir(wdpath + '\\main_results\\cfd\\mesh')
    except OSError:
        pass
